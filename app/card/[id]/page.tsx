...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const ogImage     = data.image_url
    ?? `https://www.loviaforyou.com/api/og?animal=${data.animal}&to=${encodeURIComponent(data.to)}&from=${encodeURIComponent(data.from ?? "")}`
...
export default async function CardPage({ params }: Props) {
  ...
  if (!data.opened_at) {
    ...
    await supabase.from("cards").update({ opened_at: now }).eq("id", params.id)
    data.opened_at = now
  }
